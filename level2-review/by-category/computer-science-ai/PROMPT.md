# Design the level-2 subspecialties for: Computer Science & AI

You are designing **level 2** for a scientific paper reader app. The user
already picked their level-1 category at onboarding (Computer Science & AI). On the
...
**Tagline:** Algorithms, systems, machine learning, theory.
**Covers:** AI/ML, NLP, computer vision, systems, theory of computation, HCI, security, robotics-CS; arXiv cs.*, NeurIPS, ICML, CVPR, ACL
**Audience size at level 1:** 11% of all new science papers
**Explicitly excludes (goes to other level-1 categories):** hardware → Electrical & Electronic Engineering; computational biology → Biology

## Hard constraints

- **Count: between 8 and 25 subspecialties.** Fewer than 8 feels thin
  for Computer Science & AI ("only 5 things in all of X?"). More than 25 makes the
  picker grid noisy.
- **MECE-leaning.** A paper that fits Computer Science & AI should map to exactly
...
```
{
  "level1_slug": "computer-science-ai",
  "level1_name": "Computer Science & AI",
  "count": 14,
