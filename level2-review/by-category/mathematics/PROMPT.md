# Design the level-2 subspecialties for: Mathematics

You are designing **level 2** for a scientific paper reader app. The user
already picked their level-1 category at onboarding (Mathematics). On the
...
**Tagline:** Algebra, analysis, geometry, logic, combinatorics.
**Covers:** pure math (algebra, analysis, geometry, topology, number theory, logic, combinatorics); applied math: dynamical systems, math-bio, math-physics; arXiv math.*
**Audience size at level 1:** 4% of all new science papers
**Explicitly excludes (goes to other level-1 categories):** statistical inference → Statistics & Data Science; computational complexity → Computer Science & AI

## Hard constraints

- **Count: between 8 and 25 subspecialties.** Fewer than 8 feels thin
  for Mathematics ("only 5 things in all of X?"). More than 25 makes the
  picker grid noisy.
- **MECE-leaning.** A paper that fits Mathematics should map to exactly
...
```
{
  "level1_slug": "mathematics",
  "level1_name": "Mathematics",
  "count": 14,
