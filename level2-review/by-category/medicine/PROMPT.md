# Design the level-2 subspecialties for: Medicine & Clinical Science

You are designing **level 2** for a scientific paper reader app. The user
already picked their level-1 category at onboarding (Medicine & Clinical Science). On the
next screen they see a grid of subspecialties under it. Your job is to
design that grid.

## Level-1 context

**Category:** Medicine & Clinical Science
**Tagline:** Diagnosis, surgery, therapy — bedside to bench.
**Covers:** clinical specialties (cardiology, oncology, dermatology, neurology, surgery, pediatrics, etc.); clinical trials, RCTs; NEJM, Lancet, JAMA, BMJ, specialty journals; pathology, radiology, anesthesia
**Audience size at level 1:** 16% of all new science papers
**Explicitly excludes (goes to other level-1 categories):** population studies → Public Health; wet-lab biology → Biology; drug-discovery chemistry → Chemistry

## Hard constraints

- **Count: between 8 and 25 subspecialties.** Fewer than 8 feels thin
  for Medicine & Clinical Science ("only 5 things in all of X?"). More than 25 makes the
  picker grid noisy.
- **MECE-leaning.** A paper that fits Medicine & Clinical Science should map to exactly
...
```
{
  "level1_slug": "medicine",
  "level1_name": "Medicine & Clinical Science",
  "count": 14,
