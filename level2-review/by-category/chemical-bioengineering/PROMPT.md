# Design the level-2 subspecialties for: Chemical & Bioengineering

You are designing **level 2** for a scientific paper reader app. The user
already picked their level-1 category at onboarding (Chemical & Bioengineering). On the
next screen they see a grid of subspecialties under it. Your job is to
design that grid.

## Level-1 context

**Category:** Chemical & Bioengineering
**Tagline:** Process chemistry, biotech, biomedical engineering.
**Covers:** chemical engineering, process design, biotechnology, biomedical engineering, tissue engineering, drug delivery
**Audience size at level 1:** 3% of all new science papers
**Explicitly excludes (goes to other level-1 categories):** pure chemistry → Chemistry; clinical use of devices → Medicine

## Hard constraints

- **Count: between 8 and 25 subspecialties.** Fewer than 8 feels thin
  for Chemical & Bioengineering ("only 5 things in all of X?"). More than 25 makes the
  picker grid noisy.
- **MECE-leaning.** A paper that fits Chemical & Bioengineering should map to exactly
...
```
{
  "level1_slug": "chemical-bioengineering",
  "level1_name": "Chemical & Bioengineering",
  "count": 14,
